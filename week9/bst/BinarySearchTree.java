...
//	}
	...
	private TreeNode remove(TreeNode node, DataType item) {
		...
		if (res < 0) { // The node to remove is on our left
			/**Q)) What does our recursive function return? Why does it do that?**/
			node.left = remove(node.left, item);		
		} else if (res > 0) {// The node to remove is on our right
			node.right = remove(node.right, item);	
		} else { // `node` is the node to remove
			// Case 1: The node has no children 
			if (node.left == null && node.right == null) {
				/**Q)) why does `return null` result in the node being removed?**/
				return null; // Removes this node
			}		
			// Case 2a: One child on the left
			else if (node.left != null && node.right == null) {
				/**Q)) why does returning node.left result in the node being removed?**/
				return node.left;
			}		
			...
			// Case 3: Two children
				...
				/**Q)) What is findSuccessor doing here?**/
				DataType succ = findSuccessor(node.right);
				/**Q))Why do we have a recursive remove call in this case?**/
				node.right = remove(node.right, succ);
				node.data = succ;
				return node;
			}
		}
		/**Q)) At the end of the method, why do we `return node`?**/
		return node;
	}
