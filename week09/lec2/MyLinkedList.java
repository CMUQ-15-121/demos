...
public class MyLinkedList<ListType> {
	...
	private void recursivePrint(Node<ListType> node) {
		if (node == null) {
			return;
		}
	...
	private void recursivePrintRev(Node<ListType> node) {
		if (node == null) {
			return;
		}
		recursivePrintRev(node.next);
		System.out.print(node + " ");
	}
	
	public void printListRevRecursively() {
		recursivePrintRev(this.head);
		System.out.println();
	}
